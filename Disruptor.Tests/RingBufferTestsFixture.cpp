...
namespace Tests
{
    ...
    void RingBufferTestsFixture::assertHandleResetAndNotWrap(const std::shared_ptr< RingBuffer< StubEvent > >& rb)
    {
        ...
        EXPECT_EQ(rb->hasAvailableCapacity(1), false);
    }

    void RingBufferTestsFixture::assertEmptyRingBuffer(const RingBuffer< std::any >& ringBuffer)
    {
        EXPECT_EQ(!ringBuffer[0].has_value(), true);
        EXPECT_EQ(!ringBuffer[1].has_value(), true);
        EXPECT_EQ(!ringBuffer[2].has_value(), true);
        EXPECT_EQ(!ringBuffer[3].has_value(), true);
    }
