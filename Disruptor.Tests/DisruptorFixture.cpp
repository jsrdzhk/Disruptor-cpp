...
namespace Tests
{
    ...
    {
        EXPECT_TRUE((size_t)m_sequenceLength == barrierSequences.size()) << "Should not have had any barrier sequences";
    ...
    void DisruptorFixture::assertProducerReaches(const std::shared_ptr< StubPublisher >& stubPublisher, std::int32_t expectedPublicationCount, bool strict)
    {
        ...
        else
        {
            ...
            EXPECT_TRUE(actualPublicationCount >= expectedPublicationCount) << "Producer reached unexpected count. Expected at least " << expectedPublicationCount << " but only reached " << actualPublicationCount;
        }
    }
    ...
    void DisruptorFixture::assertThatCountDownLatchIsZero(const std::shared_ptr< CountdownEvent >& countDownLatch) const
    {
        ...
        EXPECT_TRUE(released == true) << "Batch handler did not receive entries: " << countDownLatch->currentCount();
    }
